...
function commandPrepare(repo, { version, mode }) {
  ...
  // `release-version-sync.mjs` REPORTS it without ever writing it — the file is
  // generated by the template builder from FLOWCMS_VERSION, and is gitignored.
  // So the moment the bump lands, the generated copy is a version behind and
  // the verification below fails on a file no hand edited and no commit will
  // carry. Rebuilding is what `release-version-sync.mjs` tells a human to do
  // here, and there is no reason to make the human do it.
  say("  rebuilding the generated template so its derived version follows…")
  run("node", ["scripts/build-create-flowcms.mjs"])
