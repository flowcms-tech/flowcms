...
import { createBlogTagSchema, type CreateBlogTagFormValues } from '../Values/Validations'
import { ARCHIVE_INTRO_HINT, INDEXABLE_HINT, slugify } from '../Values/BlogTagValues'
import { BlogTagServices } from '../Services/BlogTagServices'
import type { BlogTag } from '../Types'

interface BlogTagCreateDrawerProps {
  isOpen: boolean
  setIsOpen: (open: boolean) => void
  onCreated: (tag: BlogTag) => void
}

export default function BlogTagCreateDrawer({ isOpen, setIsOpen, onCreated }: BlogTagCreateDrawerProps) {
  const [serverErrors, setServerErrors] = useState<string[]>([])
  const [slugTouched, setSlugTouched] = useState(false)

  const methods = useForm<CreateBlogTagFormValues>({
  ...
  const onSubmit = async (values: CreateBlogTagFormValues) => {
    setServerErrors([])
    try {
      const created = await BlogTagServices.store(values)
      onCreated(created)
      handleClose(false)
    } catch (err: unknown) {
      ...
      <FormProvider {...methods}>
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4" noValidate>
          ...
          <ElementInput
            ...
            required
            hint="Used in the tag URL. Auto-generated from the name — edit to override."
          />
