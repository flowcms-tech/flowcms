...
> **Published.** `flowcms` and `create-flowcms` are on npm at **0.2.5**, cut
> from the `v0.2.5` tag through npm Trusted Publishing, with provenance. See
...
| `flowcms` (`packages/flowcms`) | the public **theme API**, built from `src/Themes/contract` | **yes** — `flowcms@0.2.5` |
| `@example/flowcms-theme-aurora` | an example theme, used to prove the boundary | never. It is a fixture |
| `create-flowcms` (`packages/create-flowcms`) | the scaffolder that generates a site — see [create-flowcms.md](./create-flowcms.md) | **yes** — `create-flowcms@0.2.5` |
...
Both packages are published. The current release is **0.2.5**, cut from the
`v0.2.5` tag.
