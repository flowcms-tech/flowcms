...
A development patch. Under `next dev`, every save could leave a database pool
and a Redis connection open behind it, and a long editing session against MySQL
ended in `ER_CON_COUNT_ERROR: Too many connections`
([#18](https://github.com/flowcms-tech/flowcms/issues/18)). Both are now kept
for the life of the process. Production servers were never affected.

**Upgrading.** There is no schema migration and no configuration change. A
project generated by an earlier `create-flowcms` owns its copy of the source, so
it picks the fix up by copying `src/db/client.ts` and
`src/Framework/Redis/redisClient.ts` from this release; projects generated from
0.2.5 have it already.
...
[Unreleased]: https://github.com/flowcms-tech/flowcms/compare/v0.2.5...HEAD
[0.2.5]: https://github.com/flowcms-tech/flowcms/compare/v0.2.4...v0.2.5
